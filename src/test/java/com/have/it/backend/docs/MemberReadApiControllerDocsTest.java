...
class MemberReadApiControllerDocsTest extends RestDocsTestSupport {
    ...
    void 멤버_한건_조회_API_문서화_테스트() throws Exception {
        ...
        ResultActions actions =
                mockMvc
                        .perform(
                                get("/api/v1/member/read/{memberId}", memberId)
                                        .with(csrf())
    ...
    void 멤버_전체_조회_API_문서화_테스트() throws Exception {
        ...
        List<MemberReadResponse> response =
                List.of(
                        MemberReadResponse.builder()
                                .id(1L)
                                .nickname("test_response_nickname")
                                                ...
                                                builder()
                                                        ...
                                                        .responseFields(
                                                                ...
                                                                fieldWithPath("data[0].id")
                                                                        .type(JsonFieldType.NUMBER)
                                                                        .description("멤버 아이디"),
                                                                fieldWithPath("data[0].nickname")
                                                                        .type(JsonFieldType.STRING)
                                                                        .description("멤버 닉네임"),
                                                                fieldWithPath("data[0].email")
                                                                        .type(JsonFieldType.STRING)
                                                                        .description("멤버 이메일"),
                                                                fieldWithPath("data[0].role")
                                                                        .type(JsonFieldType.STRING)
                                                                        .description("멤버 권한"),
                                                                fieldWithPath("data[1].id")
                                                                        .type(JsonFieldType.NUMBER)
                                                                        .description("멤버 아이디"),
                                                                fieldWithPath("data[1].nickname")
                                                                        .type(JsonFieldType.STRING)
                                                                        .description("멤버 닉네임"),
                                                                fieldWithPath("data[1].email")
                                                                        .type(JsonFieldType.STRING)
                                                                        .description("멤버 이메일"),
                                                                fieldWithPath("data[1].role")
                                                                        .type(JsonFieldType.STRING)
                                                                        .description("멤버 권한"))
                                                        .responseSchema(Schema.schema("MemberReadResponse"))
                                                        .build())));
